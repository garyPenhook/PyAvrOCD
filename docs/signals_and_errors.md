...
| `SIGTRAP`            |                                                              | This is not an error signal. Instead, it is issued when execution stops at a place where a BREAK instruction has been placed, and it is not a breakpoint. From this point it is not possible to continue execution. |
| `SIGABRT`            | See debug server log                                         | This signal is raised after a serious internal error or when execution shall be continued after such an error. Use the `monitor info` to get more information about the error. |
