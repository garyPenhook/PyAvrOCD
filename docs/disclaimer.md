...
Further, the debug probe, the PyAvrOCD server, and the GDB debugger have a [number of limitations](limitations.md), which may lead to a behavior of the MCU that differs from when no debugger is present. Finally, when using a debugger on an MCU that controls a system in real-time, stopping the execution of the controlling program in the middle of something might lead to erroneous behavior of the controlled system.
