...
!!! info "Use dw-link as 'Arduino as ISP' programmer"
    You can use dw-link as a programmer. Select as the `Port` in the `Tools` menu the USB connection to dw-link and choose `Arduino as ISP` as the `Programmer`. Now you can upload programs and burn bootloaders and fuses.
