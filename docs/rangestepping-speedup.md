...
This means that if the first kind of sequence is observed, then one could remeber that if the next time, range-stepping is requested for the same range, we could ignore all `call`, `rcall`, and `icall` instructions. 
