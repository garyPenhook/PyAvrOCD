...
| `--elf-file`<br>`-e`                                         | ELF file that will be debugged. If the file is specified, it will be checked whether it has been compiled with the `-mrelax` option. If so, debugging is refused because the line-numbering debug information is, in this case, completely distorted. |
