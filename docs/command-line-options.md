...
| `--comm-speed`<br>`-C`                                       | Communication speed for (U)PDI in kbps (default 750). Note that values that are too low may lead to erroneous behavior of the debugger. Safe values are apparently strictly higher than 1/40 of the CPU clock. Use with care. |
