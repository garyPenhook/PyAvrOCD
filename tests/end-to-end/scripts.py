...
# set/read memory cells (small MCUs, ram start = 0x60)
    ...
    (("x/i 0x0", "0x0:\trjmp\t.+"),
     ...
     ("info reg r1","r1             0xab"),
     ("set {char}0x800005=0x56",""),
     ("set $r5=0x8c",""),
     ("info reg r5","r5             0x8c"),
     ("x/bx 0x800005","0x800005:\t0x8c"),
     ("set {char}0x80005F=0x1F",""),
     ("info reg SREG","SREG           0x1f"),
     ("x/bx 0x80005F","0x80005f:\t0x1f"),
     ("set $SREG=0xF1",""),
     ("x/bx 0x80005F","0x80005f:\t0xf1"),
     ("set $SP=0x123",""),
...
# SIGHUP will be tested in the "off" script, SIGABRT cannot be tested
     "signals" : (
      ...
      (("cont", "SIGSEGV"),
       ...
       ("cont", ""),
       ("monitor info", ""),
       ("$SUCCESS_IF", "Debugging interface:      updi"), # skip next test for UPDI targets
       ("set $sp=0x5e", ""),
       ("cont", "SIGBUS")) + epilog),

# Checks range stepping for debugWIRE targets
# _delay_ms (will be fast), loop with 2 exits (one of them a function call) will be fast,
# loop with real 2 exits will be slow, and one with 0 exits will be fast again.
# Finally, after a manual reset, we check that we indeed stop at the set (hardware) breakpoint
# and that executing the second loop is still fast.
    "range_dw" : (
    ...
    (("load",  "Start address 0x"),
     ("break range.ino:setup", "Breakpoint 1"),
     ("cont", "LedInit"),
     ("next", "LedOn"),
     ("next", "_delay_ms"),
     ("next", "LedOff"),
...
# and that executing the second loop is still fast.
    ...
    (("load",  "Start address 0x"),
     ...
     ("print cnt > 50000", "= true", "= 1"),
     ("monitor reset", "reset"),
     ("cont", "Breakpoint 2, loop"),
     ("next", "while"),
     ("next", "LedOff")) + epilog),

# Checks range stepping for JTAG targets
# Similar to above, but reserve 3 HWBPs
    "range_jtag" : (
    ...
    (("load",  "Start address 0x"),
     ...
     ("break range.ino:setup", "Breakpoint 1"),
     ("break range.ino:loop", "Breakpoint 2"),
     ("break main.cpp:main", "Breakpoint 3"),
     ("cont", "init"),
     ("cont", "LedInit"),
     ("next", "LedOn"),
     ("next", "_delay_ms"),
...
# Testing 'hardware-only' breakpoints for debugWIRE targets (1 HWBP only)
    ...
    (("load", "Start address 0x"),
     ...
     ("break blink.ino:test_cycle", "Breakpoint 4"),
     ("continue", "Breakpoint 4, test_cycle"),
     ("continue", "Breakpoint 3, loop"),
     ("next", "mydelay")) + epilog),
