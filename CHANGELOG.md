...
- **Goal reached:**
     - All e2e tests work on the AVR Dx/Ex CNANO boards and the DIP chips. There were 2 glitches (see test-problems.md, which seem not to be mine, though).

- **Added**:
     - Support for the Dx and Ex series was added (SVD, device files).
- **Fixed:**
     - Now we check for `-mrelax` in every line recorded by a producer in the ELF file in order not to miss any of those. Before, only those lines with 'GNU C' in them were tested.
...
- **Fixed:**
     - Added some wait interval of 0.5 seconds (minimum that worked was 0.3) after a switch to debug mode that followed clearing lockbits (and memory).  Otherwise, an `AVR8_FAILURE_PDI_TIMEOUT: PDI physical timed out` was thrown.
